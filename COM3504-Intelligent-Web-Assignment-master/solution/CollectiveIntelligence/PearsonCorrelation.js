...
module.exports = class PearsonCorrelation {

    // Returns the Pearson correlation coefficient for p1 and p2
    static sim(preferences, person1, person2) {

        // Get the list of shared_items
        let person1Stories = _.map(preferences[person1], (n) => {
            return _.keys(n)[0];
        });
        let person2Stories = _.map(preferences[person2], (n) => {
            return _.keys(n)[0];
        });
        ...
        _.forEach(sharedStories, (value) => {
            ...
            let person1Rating = _.values(_.find(preferences[person1], (e) => {
                if (e[value]) return e[value];
            }))[0];
            let person2Rating = _.values(_.find(preferences[person2], (e) => {
                if (e[value]) return e[value];
            }))[0];
